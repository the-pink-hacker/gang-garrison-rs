...
fn move_camera_down_system(mut query: Query<&mut Transform, With<MainCamera>>) {
    if let Ok(mut camera_transform) = query.get_single_mut() {
        camera_transform.translation += Vec3::NEG_Y * MOVE_SPEED;
    }
}

fn move_camera_up_system(mut query: Query<&mut Transform, With<MainCamera>>) {
    if let Ok(mut camera_transform) = query.get_single_mut() {
        camera_transform.translation += Vec3::Y * MOVE_SPEED;
    }
}

fn move_camera_left_system(mut query: Query<&mut Transform, With<MainCamera>>) {
    if let Ok(mut camera_transform) = query.get_single_mut() {
        camera_transform.translation += Vec3::NEG_X * MOVE_SPEED;
    }
}

fn move_camera_right_system(mut query: Query<&mut Transform, With<MainCamera>>) {
    if let Ok(mut camera_transform) = query.get_single_mut() {
